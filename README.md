...
Once downloaded, AFDKO tools are used to merge pieces of all the fonts together. The `blocks/*.txt` files specify which [Unicode Blocks](http://www.unicode.org/Public/UNIDATA/Blocks.txt) to get from which font. The `ttx` tool is used to dump the character code/glyph ID mappings for the input fonts, and `mapping.py` uses this data to create "glyph alias files" usable by AFDKO. The AFDKO `mergefonts` and `makeotf` tools can then be used to combine all the inputs into one output `KageConsoleA.otf` file that encodes all of the same characters as `HanaMinA.otf` from the Hanazono Mincho font.
